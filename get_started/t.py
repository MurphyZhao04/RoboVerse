...
states_mjx= state_tensor_to_nested(env.handler, env.handler.get_states())
print("state_mjx", states_mjx)


log.info(f"Using simulator: mujoco ")
env_class = get_sim_env_class(SimType('mujoco'))
env = env_class(scenario)
env.handler.set_states(init_states)
states_mujoco= state_tensor_to_nested(env.handler, env.handler.get_states())
print("state_mujoco", states_mujoco)
